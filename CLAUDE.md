...
**The world map is a scrollable 2D SVG+HTML layer, deliberately not a Three.js scene.** `lokace.ts` holds a hand-placed list of locations (villages, markets, dungeons, arenas, cities, one capital) as `{x, y}` percentages along a vertical strip, walked bottom-to-top like a path up a map — the same shape as a mobile gacha-RPG world map. `MapaSveta.tsx` renders that strip inside a `.mapa-platno` that scrolls (`.mapa-svet` is a fixed `2200px`-tall box, not viewport-sized — that's the actual "world size" the page scrolls through, independent of device height), with a background `<svg viewBox="0 0 100 100" preserveAspectRatio="none">` stretched to fill it: soft gradient hill/mountain blobs, a river, and a dashed path connecting every location in order. Both the path and the river are built by `cestaZBodu()`, a small quadratic-Bezier smoother (`Q ctrl, midpoint` chained into `T lastpoint`) that turns the same ordered list of points into a curve instead of straight segments — reused for the river by feeding it the same points offset a few percent away, so the two lines run alongside each other without crossing. Every pin is a plain positioned `<button>` (`left/top` in percent, matching the SVG's own coordinate space so pins stay glued to the path at any viewport width) that opens a bottom sheet on tap; the capital gets a bigger pin with its own pulse animation and a distinct message ("vyhrazené pro dějovou linku, ta zatím nevznikla") from every other location's generic "zatím nikam nevede". This was a deliberate choice over reintroducing Three.js: the map is currently just placeholder buttons, and a 2D layer is far cheaper to build, verify and re-shuffle than a 3D scene for content that doesn't exist yet — if that changes, `lokace.ts` and `cestaZBodu()` don't have to.
