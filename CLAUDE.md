...
`vite-plugin-pwa` is configured in `vite.config.ts` (manifest, Workbox precache globs, dev-mode SW enabled, `registerType: 'autoUpdate'`). The service worker is registered exactly once, via `core/utils/registerSW.ts` (`setupPWAUpdates`), called from `App.tsx`'s `useEffect` — don't add a second `registerSW()` call (e.g. in `main.tsx`), it would spin up a duplicate Workbox instance. Because `registerType` is `'autoUpdate'`, the generated service worker self-activates (`skipWaiting`/`clientsClaim`) and `virtual:pwa-register`'s `onNeedRefresh` callback is never invoked in this mode — the update happens silently (page reloads itself once the new SW takes control), so `setupPWAUpdates` only wires `onOfflineReady` for a console log, no confirm-dialog flow. `src/components/NetworkStatusBanner.tsx` + `core/hooks/useOnlineStatus.ts` show a fixed banner when `navigator.onLine` is false. There's a **second, independent update mechanism** alongside the service worker: `index.html` loads `public/js/auto-update.js` (a plain non-bundled IIFE, absolute `/js/...` path so it also resolves on deep routes like `/apps`). It polls `public/version.json` every 5 minutes and on `visibilitychange`, and hard-reloads the page when the served version differs from the one recorded when the tab loaded. Both files must live in `public/` — anything outside it isn't copied to `dist/`, which is what previously made the script and its fetch 404 in production.
