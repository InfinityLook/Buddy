...
`/hra` is being rebuilt in stages as an actual RPG layer, not the procedurally generated 3D city that used to live here (that whole `scene/` tree — Three.js city, walls, castle, forest — was deleted outright rather than kept around half-connected to a new design; nothing in it survived, since a launcher screen and an RPG have essentially nothing in common to reuse). Character creation is the first stage and is what ships today; the world map (cities, dungeons, arenas, villages, markets, a capital tied to the story) is a deliberately separate next step, not built yet — `GameModule.tsx` shows a plain placeholder screen once a character exists, on purpose, rather than a half-finished map.

**A character is chosen once and remembered, never asked again.** `useGameCharacter.ts` persists only `postavaId` (`core/utils/gameCharacterValidation.ts` validates it against the five known ids on load; an unrecognised id — corrupted storage, an old save from a removed character — resets to "no character chosen" rather than silently picking one). `GameModule.tsx` reads that id on every visit to `/hra`: null shows `TvorbaPostavy` (the picker), set skips straight past it. `postavy.ts` is the one place the five characters (Angel, Aryn, Gron, Mya, Loxen) and their bonus text live — there's no card/combat/shop system yet for the bonuses to actually apply to, so today they're descriptive text on a card, not numbers feeding a formula; when that system exists it reads from here, it doesn't get invented twice.

`App.tsx` still imports `GameModule` through `React.lazy`, left over from when the route carried Three.js — the character picker is plain DOM/CSS with nothing heavy to justify it on its own, but there was no reason to undo it either, and the map stage may well bring the weight back.
