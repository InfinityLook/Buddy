...
const tabulky = (): TabulkaSync<any>[] => {
  ...
  const wallets: TabulkaSync<Wallet> = {
    ...
    toRow: (userId, w) => ({
      ...
      name: w.name,
      icon: w.icon,
      pocatecni_zustatek: w.pocatecniZustatek,
    ...
    fromRow: (r): Wallet => ({
      ...
      name: r.name,
      icon: r.icon,
      pocatecniZustatek: Number.isFinite(r.pocatecni_zustatek) ? r.pocatecni_zustatek : 0,
      createdAt: r.created_at ?? new Date().toISOString(),
      updatedAt: zIso(r.updated_at),
      deletedAt: r.deleted_at ? zIso(r.deleted_at) : null,
    }),
