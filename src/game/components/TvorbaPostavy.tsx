...
// ==========================================
// Výběr postavy před vstupem do světa. Karetní/soubojový systém ještě
// neexistuje, takže bonusy jsou zatím jen text na kartě — až vznikne,
// bude se opírat o postavy.ts, ne o cokoliv odsud.
//
    ...
    <div className="tvorba-postavy">
      ...
      <div className="tp-mrizka">
        {POSTAVY.map((p) => (
          ...
          >
            <span className="tp-karta-ikona" aria-hidden="true">{p.ikona}</span>
            <span className="tp-karta-jmeno">{p.jmeno}</span>
            <span className="tp-karta-popis">{p.popis}</span>

            <span className="tp-bonusy">
              {p.bonusy.map((b) => (
                <span key={b} className="tp-bonus">✓ {b}</span>
              ))}
              {p.nevyhoda && <span className="tp-nevyhoda">⚠ {p.nevyhoda}</span>}
            </span>

            {vybrana === p.id && <span className="tp-znacka" aria-hidden="true">✓</span>}
          </button>
        ))}
      </div>
